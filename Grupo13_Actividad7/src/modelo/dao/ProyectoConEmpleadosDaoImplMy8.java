...
public class ProyectoConEmpleadosDaoImplMy8 extends AbstractConexionMy8 implements ProyectoConEmpleadosDao{
	...
	public double costeActualDeProyecto(String codigoProyecto) {
		sql = "select horas_asignadas * precio_hora as coste_proyecto from proyecto_con_empleados where numero_orden =?";
		double coste = 0;
		try {
			ps = conn.prepareStatement(sql);
			rs = ps.executeQuery();
	...
	public double margenActualProyecto(String codigoProyecto) {
		sql = "select ventas_previsto - coste_real as margen from proyecto_con_empleados where numero_orden =?";
		double margen = 0;
		try {
			ps = conn.prepareStatement(sql);
			rs = ps.executeQuery();
